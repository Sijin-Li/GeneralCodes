...
class BCEDiceLoss_maskloss(nn.Module):
    def __init__(self, weight=None, size_average=True):
        super().__init__()
    ...
    def forward(self, input, target, shadow, invshadow, shdweight = 0.5):
        ...
        # BCE loss
        bce_loss_shd = nn.BCELoss()(pred * shd, truth * shd).double()
        bce_loss_inv = nn.BCELoss()(pred * invshd, truth * invshd).double()

        # Dice Loss
        dice_coef_shd = (2.0 * ((pred * shd) * (truth * shd)).double().sum() + 1) / (
            pred.double().sum() + truth.double().sum() + 1
        )
        dice_coef_inv = (2.0 * ((pred * invshd) * (truth * invshd)).double().sum() + 1) / (
            pred.double().sum() + truth.double().sum() + 1
        )
