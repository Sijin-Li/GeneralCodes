...
class AttU_Netwithoutpool(nn.Module):
    """
    Attention Unet implementation
    Paper: https://arxiv.org/abs/1804.03999
    """
    def __init__(self, img_ch=3, output_ch=1):
        super(AttU_Netwithoutpool, self).__init__()
        ...
        self.downconv1 = nn.Sequential(
            nn.Conv2d(filters[0], filters[0], kernel_size=3, stride=2, padding=1, bias=True),
            nn.BatchNorm2d(filters[0]),
            nn.ReLU(inplace=True))
        self.downconv2 = nn.Sequential(
            nn.Conv2d(filters[1], filters[1], kernel_size=3, stride=2, padding=1, bias=True),
            nn.BatchNorm2d(filters[1]),
            nn.ReLU(inplace=True))
        self.downconv3 = nn.Sequential(
            nn.Conv2d(filters[2], filters[2], kernel_size=3, stride=2, padding=1, bias=True),
            nn.BatchNorm2d(filters[2]),
            nn.ReLU(inplace=True))
        self.downconv4 = nn.Sequential(
            nn.Conv2d(filters[3], filters[3], kernel_size=3, stride=2, padding=1, bias=True),
    ...
    def forward(self, x):
        ...
        e2 = self.downconv1(e1)
        e2 = self.Conv2(e2)

        e3 = self.downconv2(e2)
        e3 = self.Conv3(e3)

        e4 = self.downconv3(e3)
        e4 = self.Conv4(e4)

        e5 = self.downconv4(e4)
        e5 = self.Conv5(e5)
...
class AttU_NetwithoutConcat(nn.Module):
    """
    Attention Unet implementation
    """
    def __init__(self, img_ch=3, output_ch=1):
        super(AttU_NetwithoutConcat, self).__init__()
